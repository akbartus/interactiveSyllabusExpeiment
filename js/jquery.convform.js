// ADD TO TYPE//////////////
var messageID = 0;
function AddTypeingToMessage(id){
	
	var text = $('#message'+id).html()+'';
	console.log(text);
	if(text.indexOf('<i') !== -1)	return false; // check text has no image. TYPED TYPE ONLY TEXT
	
	$('#message'+id).text('');
	
	var typed = new Typed('#message'+id, {
	strings: [''+text],
    typeSpeed: 10,
    startDelay: 500,
    loop: false,
        ...
        if(this.current.input.hasOwnProperty('noAnswer') && this.current.input.noAnswer===true) {
            if(this.next()){
                setTimeout(function(){
                    ...
                    this.scrollDown();
                    this.printQuestion();
                }.bind(this),3000); // Change speed of appearance of the next message
            } else {
